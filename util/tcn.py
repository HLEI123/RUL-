...
class TemporalBlock(nn.Module):
  ...
  def init_weights(self):
    ...
    nn.init.kaiming_normal_(self.conv1.weight, a=0, mode='fan_in', nonlinearity='leaky_relu')
    #self.conv2.weight.data.normal_(0, 0.01)
    #nn.init.xavier_uniform(self.conv2.weight, gain=np.sqrt(2))
    nn.init.kaiming_normal_(self.conv2.weight, a=0, mode='fan_in', nonlinearity='leaky_relu')
    if self.downsample is not None:
        #self.downsample.weight.data.normal_(0, 0.01)
        #nn.init.xavier_uniform(self.downsample.weight, gain=np.sqrt(2))
        nn.init.kaiming_normal_(self.downsample.weight, a=0, mode='fan_in', nonlinearity='leaky_relu')
        
  def forward(self, x):
    net = self.net(x)
    res = x if self.downsample is None else self.downsample(x)
    return self.leaky_relu(net + res)
...
class AttentionBlock(nn.Module):
  ...
  Arguments:
      dims (int): the number of dimensions (or channels) of each element in
          the input sequence#对应in_channels
      k_size (int): the size of the attention keys#关键值注意力的关键值大小？
      v_size (int): the size of the attention values#注意力值的大小？
      seq_len (int): the length of the input and output sequences#输入、输出长度：timestep？
  """
